...
import { Sun, Moon, Bell, Settings, MessageSquare, PanelLeftClose, PanelRightClose, PanelLeft, PanelRight, Download, Sliders, Layers, BookOpen, Ruler, Save, Database } from 'lucide-react';
import { useTranslation } from 'react-i18next';
const TopBar = ({ onBaseMapsClick, onExportClick, onFiltersClick, onResearchClick, onMeasureClick, onDownloadClick, onProjectClick, onQueryClick }) => {
  ...
  const btnStyle = { background: 'transparent', border: 'none', cursor: 'pointer', padding: '8px', borderRadius: '6px', color: color, display: 'flex', alignItems: 'center' };
  return (
    <div style={{ height: '60px', background: bg, backdropFilter: 'blur(10px)', borderBottom: '1px solid ' + border, display: 'flex', alignItems: 'center', padding: '0 20px', gap: '12px', color: color, position: 'relative', zIndex: 1000 }}>
      ...
      <SearchBar />
      <div style={{ flex: 1 }} />
      <select onChange={(e) => changeLanguage(e.target.value)} defaultValue="en" style={{ padding: '6px 12px', borderRadius: '6px', border: '1px solid ' + border, background: theme === 'dark' ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)', color: color, cursor: 'pointer', fontSize: '13px' }}>
        <option value="en">EN</option>
        <option value="ar">AR</option>
        <option value="es">ES</option>
      </select>
      <button onClick={onQueryClick} style={btnStyle} title="Spatial Query"><Database size={20} /></button>

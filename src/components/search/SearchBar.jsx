...
const SearchBar = () => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      import('../../data/administrativeDivisions.json').then(m => m.default)
  ...
  useEffect(() => {
    ...
    allData.countries.forEach(f => {
      ...
      if (name.includes(query) || capital.includes(query)) {
        results.push({
          ...
          subtitle: `Country • Capital: ${f.properties.capital}`,
          feature: f,
          icon: Globe
        });
      }
    ...
    allData.divisions.forEach(f => {
      ...
      if (name.includes(query) || (country.includes(query) && name.length > 0)) {
        results.push({
          type: 'division',
          name: f.properties.name,
          subtitle: `${f.properties.type} • ${f.properties.country}`,
          feature: f,
          icon: Building2
        });
      }
  ...
  const handleSelect = (result) => {
    ...
    if (result.type === 'country' || result.type === 'division') {
      ...
      window.dispatchEvent(new CustomEvent('flyToBounds', { 
        detail: { bounds, padding: 50, maxZoom: 8, duration: 1500 } 
      }));
      ...
      window.dispatchEvent(new CustomEvent('flyToLocation', { 
        detail: { center: coords, zoom: 10, duration: 1500 } 
      }));
    }
  };
  ...
  return (
    <div style={{ position: 'relative', width: '350px' }}>
      <div style={{
        ...
        <input
          type="text"
          placeholder="Search countries, states, capitals..."
          value={searchQuery}
      ...
      {isOpen && searchResults.length > 0 && (
        <div style={{
          ...
          {searchResults.map((result, idx) => {
            ...
            return (
              <div key={idx} onClick={() => handleSelect(result)} style={{
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '13px', fontWeight: 600, color }}>{result.name}</div>
